...
int ngbe_phy_write_reg_exphy(struct ngbe_hw *hw, u16 reg_offset,
...
int ngbe_phy_read_reg_exphy(struct ngbe_hw *hw, u16 reg_offset,
			      u16 page, u16 *phy_data)
{
	unsigned long flags;
...
static int ngbe_check_ex_generic_phy_id(struct ngbe_hw *hw)
{
	...
	ngbe_phy_read_reg_exphy(hw, NGBE_MDI_PHY_ID1_OFFSET, 0, &phy_id1);
	ngbe_phy_read_reg_exphy(hw, NGBE_MDI_PHY_ID2_OFFSET, 0, &phy_id2);

	if (phy_id1 == NGBE_JL2201_PHY_ID1 &&
	 	phy_id2 == NGBE_JL2201_PHY_ID2) {
		hw->phy.id = phy_id1;
...
int ngbe_phy_init(struct ngbe_hw *hw)
{
	...
	if (hw->phy.type == ngbe_phy_internal || hw->phy.type == ngbe_phy_internal_yt8521s_sfi) {
		...
		hw->phy.ops.read_reg_mdi(hw, 0x12, 0, &value);
		...
		if (hw->phy.phydev_type == ngbe_phy_jl) {
			...
			ngbe_phy_write_reg_exphy(hw, 16, 158, value | (1 << 14));

			ngbe_phy_read_reg_exphy(hw, 21, 160, &value);
			ngbe_phy_write_reg_exphy(hw, 21, 160, value | (1 << 11));

			ngbe_phy_read_reg_exphy(hw, RTL821x_INER, 0xa42, &value);
			ngbe_phy_write_reg_exphy(hw, RTL821x_INER, 0xa42, value | RTL8211F_INER_LINK_CHANGE);
			...
			ngbe_phy_write_reg_exphy(hw, RTL821x_INER, 0xa42, RTL8211F_INER_LINK_CHANGE);
			ngbe_phy_write_reg_exphy(hw, 0x11, 0xde1, RTL8211F_INER_LINK_CHANGE);

			/*
			value = RTL8211F_TX_DELAY;
			ngbe_phy_write_reg_exphy(hw, 0x11, 0xd08, value);
			*/
		...
		/* power down */
		ngbe_phy_read_reg_exphy(hw, RTL8211F_MII_BMCR, 0, &value);
		value |= RTL8211F_BMCR_PDOWN;
		ngbe_phy_write_reg_exphy(hw, RTL8211F_MII_BMCR, 0, value);
	}
...
u32 ngbe_phy_setup_link_ex_generic(struct ngbe_hw *hw,
				   u32 speed,
				   bool autoneg_wait_to_complete)
{
	u16 value = 0;
...
int ngbe_phy_check_event_ex(struct ngbe_hw *hw)
{
	u16 value = 0;
	struct ngbe_adapter *adapter = hw->back;

	if (hw->phy.phydev_type == ngbe_phy_jl) {
		ngbe_phy_read_reg_exphy(hw, RTL821x_INSR, 0xa43, &value);
	} else {
		ngbe_phy_read_reg_exphy(hw, RTL821x_INSR, 0xa43, &value);
...
int ngbe_phy_get_advertised_pause_rtl821x(struct ngbe_hw *hw, u8 *pause_bit)
{
	u16 value = 0;
	int status = 0;
...
int ngbe_phy_get_lp_advertised_pause_rtl821x(struct ngbe_hw *hw, u8 *pause_bit)
{
	u16 value = 0;
	int status = 0;
...
int ngbe_phy_set_pause_advertisement_rtl821x(struct ngbe_hw *hw,
												u16 pause_bit)
{
	u16 value = 0;
	int status = 0;
...
int ngbe_exphy_suspend(struct ngbe_hw *hw)
{
	struct ngbe_adapter *adapter = hw->back;
	u16 value;
