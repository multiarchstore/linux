...
#if 0
static s32 txgbe_setup_mac_link_hostif(struct txgbe_hw *hw, u32 speed)
{
...
#if 0
static int txgbe_flash_write_cab(struct txgbe_hw *hw,u32 addr, u32 value,u16 lan_id)
{
	int status;
...
static int txgbe_flash_read_cab(struct txgbe_hw *hw, u32 addr ,u16 lan_id )
{
	int status;
	struct txgbe_hic_read_cab buffer;
...
#if 0
static int txgbe_flash_write_lock(struct txgbe_hw *hw)
{
	int status;
	struct txgbe_hic_read_shadow_ram buffer;
...
s32 txgbe_set_link_to_kr(struct txgbe_hw *hw, bool autoneg)
{
	...
	if (AN73_TRAINNING_MODE == 1 ||
	    AN73_TRAINNING_MODE == 2 ||
	    AN73_TRAINNING_MODE == 3)
